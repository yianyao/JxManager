...
class AuthGroupModel extends Model {
    ...
    public function addToGroup($uid,$gid){
        $gc = M(self::AUTH_GROUP_ACCESS);
        /**
         * 检查表中是否已有当前用户和角色相对应的记录
        **/
        if($gc->where(array('uid'=>$uid,'group_id'=>$gid))->find()){
            return -1;
        }else{
            //当前用户没有授权，则添加新授权
            $data['uid'] = $uid;
    ...
    /**
     * 解除用户授权
     * @param array $deleteItems : 解除条件
     * @return int|boolean
    **/
    public function deleteRole($deleteItems)
    {
        $m = D(self::AUTH_GROUP_ACCESS);
        $gid = M(self::AUTH_GROUP)->getField('id,title');
        $where = array();
        $map = array();
        foreach($deleteItems as $k=>$v){
            foreach($gid as $key=>$value){
                if(strtolower($v['gid']) == $value){
                    $where['group_id'] = $key;
                }
            }
            $map[] = "(`uid` = " . $v['uid'] . ' AND ' . "`group_id`= " . $where['group_id'] . ")";
        }
        $where = implode(' OR ',$map);
