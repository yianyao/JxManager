var Datatable = function() {
    ...
    return {
        ...
        deleteRow : function(it) {
            ...
            if (it&&it.length) {
                if (it.length == 1) {
                    ...
                    message = "确定要删除选中的"+it.length+"项记录吗?";
                }
                layer.confirm(message, function(){
                    $.ajax({
                        ...
                        success:function(res){
                            if (res > 0){
                                layer.msg("已删除" + res + "条记录！");
                                dataTable.ajax.reload();
                            }else if (res === 0){
                                layer.msg('没有删除任何数据！');
                            }else if(res === false){
                                layer.msg("SQL出错，没有删除数据！")
                            }
                        },
                        error:function(e){
                            layer.msg('无法执行删除操作！');
        ...
        editRow:function(rowInfo){
            $('#username-edit').val(rowInfo.nname);
            $('#userid').val(rowInfo.uid);
            layer.open({
                type: 1,
                title: '用户角色修改',
                move:'.layui-layer-title',
